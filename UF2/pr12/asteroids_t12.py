...
def asCol():
    for y in [x for x in world.sprites if isinstance(x, Asteroid)]:
        for z in [x for x in world.sprites if isinstance(x, Bullet)]:
            if(abs(y.rect.center[0] - z.rect.center[0]) < 15 and abs(y.rect.center[1] - z.rect.center[1]) < 15):
                y.duration = 0
                z.duration = 0
                y.kill()
                z.kill()
...
def deathCol():
    for y in [x for x in world.sprites if isinstance(x, Player)]:
        for z in [x for x in world.sprites if isinstance(x, Asteroid)]:
            if(abs(y.rect.center[0] - z.rect.center[0]) < 15 and abs(y.rect.center[1] - z.rect.center[1]) < 15):
...
def main():
    ...
    supdate.start()
    supdate = Thread(target=update_collision)
    supdate.start()
    while world.running:
